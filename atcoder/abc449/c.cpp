...
#ifdef LOCAL
#include "test.hpp"
#define RUN_TESTS() run_local_tests(argc, argv, gen, brute, false)
#else
#define RUN_TESTS()
#endif
// clang-format on
...
void solve(int TC) {
    ...
    for (int i = 0; i < s.size(); ++i) {
        ...
        auto lit = std::lower_bound(v[idx].begin(), v[idx].end(), i + l);
        auto rit = std::lower_bound(lit, v[idx].end(), i + r);

        if (lit == v[idx].end())
            continue;

        if (rit == v[idx].end() || *rit > i + r)
            --rit;
